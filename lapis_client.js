...
for(let i in tokenizer.body) {
    ...
    if(token.type == 'MustacheStatement') {
        ...
        const githubAccount = parts.shift();
        const githubRepo = parts.shift();
        const mdPath = parts.join(".");

        // Build tokens object that we will loop on it to get variables
        tokens[githubAccount] = tokens[githubAccount] || {};
        tokens[githubAccount][githubRepo] = tokens[githubAccount][githubRepo] || {};
        tokens[githubAccount][githubRepo][mdPath] = tokens[githubAccount][githubRepo][mdPath] || {};
        tokens[githubAccount][githubRepo][mdPath][variableName] = "";
    }
}

// Get md pages
promises = [];
for (let githubAccount in tokens) {
    for (let githubRepo in tokens[githubAccount]) {
        for (let mdPath in tokens[githubAccount][githubRepo]) {
            let url = `${BASE_URL}/${githubAccount}/${githubRepo}/master/${mdPath.replace(/\./g, "/")}.md`;
            promises.push(makeRequest(url, tokens[githubAccount][githubRepo][mdPath]));
        }
    }
}
